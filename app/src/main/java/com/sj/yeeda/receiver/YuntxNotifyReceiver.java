...
public class YuntxNotifyReceiver extends ECNotifyReceiver {
    ...
    public void showToast(ECMultiDeviceState deviceState) {
        ...
        ToastUtils.showShortToast(stringBuilder.toString());
    ...
    public void onNotificationClicked(Context context, int notifyType, String sender) {
        Logger.d("ECSDK_Demo.YuntxNotifyReceiver", "onNotificationClicked notifyType " + notifyType + " ,sender " + sender);
...
//        context.startActivity(intent);
    ...
    public void onSoftVersion(Context context, String version, String softDesc, boolean force) {
        ...
        Logger.e("aaaa", version + softDesc + force);
//        SDKCoreHelper.setSoftUpdate(version, softDesc, force);
    ...
    public static class NotifyService extends Service {
        ...
        private void receiveImp(Intent intent) {
            if (intent == null) {
                Logger.e(TAG, "receiveImp receiveIntent == null");
                return;
            }
            Logger.i(TAG, "intent:action " + intent.getAction());
            int optCode = intent.getIntExtra(SERVICE_OPT_CODE, 0);
            if (optCode == 0) {
                Logger.e(TAG, "receiveImp invalid opcode.");
                return;
            }
            ...
            switch (optCode) {
                case EVENT_TYPE_CALL:
                    Logger.d(TAG, "receive call event ");
...
//                    startActivity(call);
        ...
        public int onStartCommand(Intent intent, int flags, int startId) {
            Logger.v(TAG, String.format("onStartCommand flags :%d startId :%d intent %s", flags, startId, intent));
            receiveImp(intent);
        ...
        private void dispatchOnReceiveMessage(Intent intent) {
            if (intent == null) {
                Logger.e(TAG, "dispatch receive message fail.");
                return;
            }
